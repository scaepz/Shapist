...
bool CRocketLauncher::Attack(int x, int y, float dirX, float dirY)
{

	//calculate starting x and y speed
	if (dirX == 0) dirX = 0.01f;
	if (dirY == 0) dirY = 0.01f;
	float a = sqrt(dirX*dirX + dirY*dirY) / projectileSpeed;
	float speedX = dirX / a;
	float speedY = dirY / a;
	...
	CRocket * rocket = new CRocket(xPos, y + correctedOffsetY + rocketHeight / 302, 1002, 44, 10, 8, 150, rocketWidth, rocketHeight, speedX, speedY, true, 10000, 400, 1, 110);
