			...
			if (renderLap == 3 && !renderedWeapons)
			{
				for (int a = 0; a < animateVector->size(); a++)
				{
					...
					if (animateVector->at(a)->GetWeaponEquipped() >= 0)
					{
						...
						{
							if (animateVector->at(a)->currentAnim != 0) //if obj is not idle
							{

								dstrect.x = animateVector->at(a)->pos[0] + animateVector->at(a)->kickback->GetX()*kickBackFlipper + (animateVector->at(a)->GetWidth() / 2) - screenPos[0];
								dstrect.y = animateVector->at(a)->pos[1] + animateVector->at(a)->kickback->GetY()*kickBackFlipper + (animateVector->at(a)->GetHeight() / 2) - screenPos[1];
							...
							{
								dstrect.x = animateVector->at(a)->pos[0] - screenPos[0];
								dstrect.y = animateVector->at(a)->pos[1] + (animateVector->at(a)->GetHeight() / 3) + 8 - screenPos[1];
								SDL_RenderCopyEx(renderer,
									textureMap[weapon->textureId],
									NULL,
									&dstrect,
						...
						else
						{
							...
							srcrect.x = (weapon->GetFrame() % 4) * srcrect.w;
							srcrect.y = (weapon->GetFrame() / 4) * srcrect.h;


							if (animateVector->at(a)->currentAnim != 0)
							{
								dstrect.x = animateVector->at(a)->pos[0] + animateVector->at(a)->kickback->GetX()*kickBackFlipper + (animateVector->at(a)->GetWidth() / 2) - screenPos[0];
								dstrect.y = animateVector->at(a)->pos[1] + animateVector->at(a)->kickback->GetY()*kickBackFlipper + (animateVector->at(a)->GetHeight() / 2) - screenPos[1];
...
void CArtist::DrawBackground(background bg)
{
	...
	dest.w = bg.width;
	dest.h = bg.height;
	for (int x = screenPos[0] / resolution[0]; x <= (screenPos[0] / resolution[0]) + (resolution[0] / bg.width) + 1; x++)
	{
		dest.x = (x*bg.width) - (float)screenPos[0] * bg.distanceMultiplier;
		for (int y = screenPos[1] / resolution[1] - 1; y <= (screenPos[1] / resolution[1]) + (resolution[1] / bg.height) + 1; y++)
		{
			dest.y = (y*bg.height) - (float)screenPos[1] * bg.distanceMultiplier;
