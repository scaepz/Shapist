...
bool CMeleeWeapon::Attack(int x, int y, float dirX, float dirY)
{
	if (vm->GetPlayer()->LoseStamina(staminaCost));
	{
		StartAnimation(rand() % 2 + 1);
...
int CMeleeWeapon::GetFrame()
{

	if (timeSinceAnimationStart / animationData[currentAnimation][1] < animationData[currentAnimation][frameCountData])
	{
		return animationData[currentAnimation][startFrameData] + timeSinceAnimationStart / animationData[currentAnimation][msPerFrameData];
	}
	else
	{
